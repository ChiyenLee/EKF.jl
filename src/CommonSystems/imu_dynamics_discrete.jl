...
#                        State Definitions
###############################################################################
struct TrunkState{T} <: EKF.State{16,T}
	x::T; y::T; z::T
	vx::T; vy::T; vz::T
	qw::T; qx::T; qy::T; qz::T
	αx::T; αy::T; αz::T
...
struct TrunkError{T} <: EKF.ErrorState{15,T}
	𝕕x::T; 𝕕y::T; 𝕕z::T
	𝕕vx::T; 𝕕vy::T; 𝕕vz::T
	𝕕ϕx::T; 	𝕕ϕy::T; 	𝕕ϕz::T
	𝕕αx::T; 	𝕕αy::T ;	𝕕αz::T
...
struct Vicon{T} <: EKF.Measurement{7,T}
	x::T; y::T; z::T
	qw::T; qx::T; qy::T; qz::T
end
...
function EKF.process(s::TrunkState, u::ImuInput, h::Float64)
	...
	C = Rotations.UnitQuaternion(s.qw, s.qx, s.qy, s.qz) # from body to world

	rₖ₊₁ = r + h*v + 0.5*h^2*(C*(f-α)-g)
	vₖ₊₁ = v + h*(C*(f - α) - g)
	qₖ₊₁ = q + 0.5 * Rotations.∇differential(C) * (ω - β) * h  #L(q) * ζ((ω-state.βω)*h)
	qₖ₊₁ = qₖ₊₁ / norm(qₖ₊₁)
...
function EKF.error_process_jacobian(s::TrunkState, u::ImuInput, h::Float64)
	sₖ₊₁ₗₖ = EKF.process(s,u,h) # not ideal to call it again here but oh well
	qₖ = Rotations.UnitQuaternion(s.qw, s.qx, s.qy, s.qz)
	...
	Jₖ[7:10, 7:9] .= Rotations.∇differential(qₖ)
	Jₖ[1:6, 1:6] .= I6;  Jₖ[11:16, 10:15] .= I6;
	Jₖ₊₁ₗₖ[7:10, 7:9] .= Rotations.∇differential(qₖ₊₁ₗₖ)
	Jₖ₊₁ₗₖ[1:6, 1:6] .= I6;  Jₖ₊₁ₗₖ[11:16, 10:15] .= I6;

	Jₖ = SMatrix(Jₖ)
	Jₖ₊₁ₗₖ = SMatrix(Jₖ₊₁ₗₖ)
    F = ForwardDiff.jacobian(st->EKF.process(TrunkState(st), u, h), SVector(s))
	return Jₖ₊₁ₗₖ' * F * Jₖ
end
...
function EKF.error_measure_jacobian(s::TrunkState)
	...
	Jₓ = Rotations.∇differential(Rotations.UnitQuaternion(s.qw, s.qx, s.qy, s.qz))
...
function EKF.measurement_error(m2::Vicon, m1::Vicon)
	...
	q1 = Rotations.UnitQuaternion(q1)
	q2 = Rotations.UnitQuaternion(q2)
	dr = r2 - r1
