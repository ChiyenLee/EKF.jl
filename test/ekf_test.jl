...
@testset "Mulitplicative Extended Kalman Filter" begin
    ...
    function EKF.process(xₖ::OriVel, uₖ::Tor, dt::Float64)
        # No torque applied just kinematics
        q′ = Rotations.kinematics(UnitQuaternion(xₖ.q𝑤, xₖ.q𝑥, xₖ.q𝑦, xₖ.q𝑧), dt*SA[xₖ.ω𝑥, xₖ.ω𝑦, xₖ.ω𝑧])
        ω = SA[xₖ.ω𝑥, xₖ.ω𝑦, xₖ.ω𝑧]
        return OriVel([q′;  ω])
    end
