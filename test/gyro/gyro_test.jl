...
include("$(@__DIR__)/gyro_states.jl")
...
measure_cov = Matrix(.1 * I(length(QuatErrorMeasurement)))
...
for imu_row in eachrow(imu_df[2:end, :])
    ...
    state, _ = estimateState!(ekf, input, measurement, .1);
    push!(data, copy(state))
